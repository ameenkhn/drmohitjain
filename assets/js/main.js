...
/* ===== Animation failsafe (site-wide) =====
   If entrance animations never initialise (script error, blocked asset, etc.),
   reveal all content so nothing is left invisible. */
(function () {
  function reveal() { document.body.classList.add('no-anim'); }
  window.setTimeout(function () {
    var probe = document.querySelector('.text-anime-style-3, [data-aos]');
    if (!probe) return;
    if (parseFloat(getComputedStyle(probe).opacity) < 0.1) reveal();
  }, 2600);
